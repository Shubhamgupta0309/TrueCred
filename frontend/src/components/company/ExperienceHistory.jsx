...
export default function ExperienceHistory({ history = [], onDelete }) {
  ...
  const handleDelete = async (item) => {
    ...
    const confirmed = window.confirm('Delete this history item? This also removes it from student data.');
    if (!confirmed) return;
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-xl bg-slate-950 border border-cyan-500/30 rounded-2xl p-6 text-cyan-100">
            ...
            <div className="mt-4 space-y-2 text-sm">
              <p><span className="text-cyan-300">Title:</span> {selectedItem.experienceTitle}</p>
              <p><span className="text-cyan-300">Student:</span> {selectedItem.studentName}</p>
